...
DocumentHandler.prototype.handleGet = function(key, response, skipExpire) {
  this.store.get(key, function(ret) {
    if (ret) {
      ...
      response.end(JSON.stringify({ data: (key === 'about' ? ret : JSON.stringify(JSON.parse(ret), null, 4)), key: key }));
    }
    else {
...
DocumentHandler.prototype.handlePost = function (request, response) {
  ...
  var onSuccess = function () {
    ...
    try {
      ...
      winston.verbose('Error non-json document');
      response.writeHead(400, { 'content-type': 'application/json' });
      response.end(JSON.stringify({ message: 'Error non-json document.' }));
      return;
    }
